	...
	LPARAM lParam)
{
	...
	static bool draw1 = false;
	static bool draw2 = false;
	static bool draw3 = false;
	int i=0, j = 0;
	switch (iMessage)
	{
	case WM_CREATE: 
	...
	case WM_LBUTTONDOWN:
	{
			...
			if ((rc[currentBox].right - rc[currentBox].left) / 2 + rc[currentBox].left > pt_x && rc[currentBox].left < pt_x &&
				(rc[currentBox].bottom - rc[currentBox].top) / 2 + rc[currentBox].top > pt_y && rc[currentBox].top < pt_y)
			{
				draw = true;
			}
			if ((rc[currentBox].right - rc[currentBox].left) / 2 + rc[currentBox].left < pt_x && rc[currentBox].right > pt_x &&
				(rc[currentBox].bottom - rc[currentBox].top) / 2 + rc[currentBox].top > pt_y && rc[currentBox].top < pt_y)
			{
				draw1 = true;
			}
			if ((rc[currentBox].right - rc[currentBox].left) / 2 + rc[currentBox].left > pt_x && rc[currentBox].left < pt_x &&
				(rc[currentBox].bottom - rc[currentBox].top) / 2 + rc[currentBox].top < pt_y && rc[currentBox].bottom > pt_y)
			{
				draw2 = true;
			}
			if ((rc[currentBox].right - rc[currentBox].left) / 2 + rc[currentBox].left < pt_x && rc[currentBox].right > pt_x &&
	...
	{			
		...
		if (draw == true)
		{
			...
			if (tempx < pt_x || tempx > pt_x)
			{
				rc[currentBox].left = rc[currentBox].left - (pt_x - tempx);
				pt_x = tempx;
			}
			if (tempy < pt_y || tempy > pt_y)
			{
